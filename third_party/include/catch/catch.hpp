...
namespace Catch {
    ...
    private:
        ...
        template<typename T>
        void addPattern() {
            std::string m_token = subString();
            for( std::size_t i = 0; i < m_escapeChars.size(); ++i )
                m_token = m_token.substr( 0, m_escapeChars[i]-m_start-i ) + m_token.substr( m_escapeChars[i]-m_start-i+1 );
            m_escapeChars.clear();
            if( startsWith( m_token, "exclude:" ) ) {
                m_exclusion = true;
                m_token = m_token.substr( 8 );
            }
            if( !m_token.empty() ) {
                TestSpec::PatternPtr pattern = std::make_shared<T>( m_token );
                if( m_exclusion )
...
namespace __detail {
    ...
    // Wraps a m_token coming from a m_token stream. These may not directly correspond to strings as a single string
    ...
    public:
        ...
        auto parse( std::string const &, TokenStream const &tokens ) const -> InternalParseResult override {
            ...
            auto const &m_token = *remainingTokens;
            if( m_token.type != TokenType::Argument )
                return InternalParseResult::ok( ParseState( ParseResultType::NoMatch, remainingTokens ) );

            auto result = m_ref->setValue( remainingTokens->m_token );
            if( !result )
            ...
            if( remainingTokens && remainingTokens->type == TokenType::Option ) {
                ...
                if( isMatch(m_token.m_token ) ) {
                    if( m_ref->isFlag() ) {
                        ...
                        ++remainingTokens;
                        if( !remainingTokens )
                            return InternalParseResult::runtimeError( "Expected argument following " + m_token.m_token );
                        auto const &argToken = *remainingTokens;
                        if( argToken.type != TokenType::Argument )
                            return InternalParseResult::runtimeError( "Expected argument following " + m_token.m_token );
                        auto result = m_ref->setValue( argToken.m_token );
                        if( !result )
                            return InternalParseResult( result );
                        if( result.value() == ParseResultType::ShortCircuitAll )
                            return InternalParseResult::ok( ParseState( result.value(), remainingTokens ) );
        ...
        auto parse( std::string const& exeName, TokenStream const &tokens ) const -> InternalParseResult override {
            ...
            while( result.value().remainingTokens() ) {
                ...
                if( result.value().type() == ParseResultType::ShortCircuitAll )
                    return result;
                if( !tokenParsed )
                    return InternalParseResult::runtimeError( "Unrecognised m_token: " + result.value().remainingTokens()->m_token );
            }
...
using __detail::ExeName;

// Convenience wrapper for option parser that specifies the help option
using __detail::Help;

// enum of result types from a parse
using __detail::ParseResultType;

// Result type for parser operation
using __detail::ParserResult;
...
namespace Catch {
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto const setRngSeed = [&]( std::string const& seed ) {
                if( seed != "time" )
                    return clara::__detail::convertInto( seed, config.rngSeed );
...
namespace Catch {
    ...
    void enforceNoDuplicateTestCases( std::vector<TestCase> const& functions ) {
        ...
        for( auto const& function : functions ) {
            auto m_prev = seenFunctions.insert( function );
            CATCH_ENFORCE( m_prev.second,
                    "error: TEST_CASE( \"" << function.name << "\" ) already defined.\n"
                    << "\tFirst seen at " << m_prev.first->getTestCaseInfo().lineInfo << "\n"
